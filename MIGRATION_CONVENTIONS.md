...
> **This product's schema is `campaign`.** Its migration ledger is
> `campaign.geiger_migrations`. Substitute `campaign` wherever this doc writes
...
(`campaign.tickets`), never `campaign.flow_tickets`.
...
-- Add seat maps
--
-- Owns campaign.seat_maps. Free-form header: what this changes and why.

-- @up
create table if not exists campaign.seat_maps (
...
-- @down
drop table if exists campaign.seat_maps;
```
...
grant usage on schema campaign to anon, authenticated, service_role;

-- Define the shared trigger function locally rather than depending on another
-- migration having created it.
create or replace function campaign.touch_updated_at()
returns trigger language plpgsql as $$
begin new.updated_at = now(); return new; end;
$$;

create table if not exists campaign.tickets (...);
alter table campaign.tickets add column if not exists tier text;   -- back-fill older copies
create index if not exists tickets_event_idx on campaign.tickets (event_id);

drop trigger if exists tickets_touch_updated_at on campaign.tickets;
create trigger tickets_touch_updated_at
before update on campaign.tickets
for each row execute function campaign.touch_updated_at();

drop policy if exists tickets_demo_all on campaign.tickets;
create policy tickets_demo_all on campaign.tickets for all to anon, authenticated
...
```sql
alter table campaign.tickets enable row level security;
drop policy if exists tickets_demo_all on campaign.tickets;
create policy tickets_demo_all on campaign.tickets for all to anon, authenticated
  using (true) with check (true);
```
...
-- @no-transaction
-- @up
create index concurrently if not exists tickets_event_idx on campaign.tickets (event_id);
```
