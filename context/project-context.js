...
// Active-project context for the Campaign workspace. The whole dashboard is
// scoped to one project at a time: every data-layer call is filtered by the
// active project's id and RLS enforces org membership on top.
//
// The active project lives in the URL (/project/<uuid>/…, via useWorkspaceUrl)
// so a refresh or shared link stays on it; localStorage only remembers the last
// choice to pre-select it on a fresh visit. Projects themselves are the shared
// suite entity public.projects — read through a public-schema client, narrowed
// to the caller's orgs by that table's own RLS.

const ProjectContext = createContext(undefined);

const LAST_PROJECT_KEY = "geiger-campaign:last-project";
...
// RPC live in the shared public schema, not this app's own schema.
...
export function ProjectProvider({ children }) {
  ...
  // needed), then select it. Mirrors the suite's project creation.
