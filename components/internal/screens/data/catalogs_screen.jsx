      ...
      <DialogContent>
        ...
        <DialogBody className="space-y-4 py-4">
          <Field label="Catalog name" htmlFor="c-name">
            <Input id="c-name" value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} placeholder="e.g. Storefront products" className="bg-[#161616] border-[#2a2a2a]" />
          </Field>
          ...
          <TableBody>
            {filtered.map((c) => (
              <TableRow key={c.id} className="border-[#2a2a2a]">
                ...
                <TableCell className="tabular-nums text-[#a3a3a3]">{c.items.toLocaleString()}</TableCell>
                <TableCell className="whitespace-nowrap text-[#a3a3a3]">{c.source}</TableCell>
                <TableCell className="whitespace-nowrap text-[#a3a3a3]">{c.lastSync}</TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow className="border-[#2a2a2a] hover:bg-transparent">
                <TableCell colSpan={7} className="py-14 text-center text-sm text-[#737373]">No catalogs found.</TableCell>
              </TableRow>
            )}
          </TableBody>
        </Table>
